...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <yobaaddress> <amount>\n"
            "yobaaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <yobaaddress> <amount>\n"
            "yobaaddress, denominate, or auto (AutoDenominate)"
            "<amount> is type \"real\" and will be rounded to the nearest 0.1"
            + HelpRequiringPassphrase());

    CYobaCoincoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid yoba address");
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in yoba.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in yoba.conf\n"
                ...
                "  vote-many    - Vote on a YobaCoin initiative\n"
                "  vote         - Vote on a YobaCoin initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CYobaCoincoinSecret(secret).ToString();
    }
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a YobaCoin initiative (can be additionally filtered, partial match)\n"
