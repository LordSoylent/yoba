...
CScript _createmultisig(const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        CYobaCoincoinAddress address(ks);
...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CYobaCoincoinAddress address(innerID);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new YobaCoin address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"yobaaddress\"    (string) The new YobaCoin address\n"
            "\nExamples:\n"
    ...
    return CYobaCoincoinAddress(keyID).ToString();
}


CYobaCoincoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CYobaCoincoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current YobaCoin address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"yobaaddress\"   (string) The account YobaCoin address\n"
            "\nExamples:\n"
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"yobaaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"yobaaddress\"  (string, required) The YobaCoin address to be associated with an account.\n"
            "2. \"account\"         (string, required) The account to assign the address to.\n"
            "\nExamples:\n"
            + HelpExampleCli("setaccount", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\" \"tabby\"")
            + HelpExampleRpc("setaccount", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\", \"tabby\"")
        );

    CYobaCoincoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid YobaCoin address");
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"yobaaddress\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"yobaaddress\"  (string, required) The YobaCoin address for account lookup.\n"
            "\nResult:\n"
            "\"accountname\"        (string) the account address\n"
            "\nExamples:\n"
            + HelpExampleCli("getaccount", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\"")
            + HelpExampleRpc("getaccount", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\"")
        );
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"yobaaddress\"  (string) a YobaCoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CYobaCoincoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CYobaCoincoinAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"yobaaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSent an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
           "1. \"yobaaddress\"  (string, required) The YobaCoin address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in YOBA to send. eg 0.1\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("sendtoaddress", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\" 0.1")
            + HelpExampleCli("sendtoaddress", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\" 0.1 \"reward\" \"seans outpost\"")
            + HelpExampleRpc("sendtoaddress", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\", 0.1, \"reward\", \"seans outpost\"")
        );
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"yobaaddress\",     (string) The YobaCoin address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CYobaCoincoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CYobaCoincoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CYobaCoincoinAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"yobaaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"yobaaddress\"  (string, required) The YobaCoin address to use for the private key.\n"
            ...
            + HelpExampleCli("signmessage", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\" \"my message\"") +
            "\nVerify the signature\n"
            + HelpExampleCli("verifymessage", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\" \"signature\" \"my message\"") +
            "\nAs json rpc\n"
            + HelpExampleRpc("signmessage", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\", \"my message\"")
        );
    ...
    CYobaCoincoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"yobaaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given yobaaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"yobaaddress\"  (string, required) The YobaCoin address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount   (numeric) The total amount in YOBA received at this address.\n"
            "\nExamples:\n"
            "\nThe amount from transactions with at least 1 confirmation\n"
            + HelpExampleCli("getreceivedbyaddress", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\"") +
            "\nThe amount including unconfirmed transactions, zero confirmations\n"
            + HelpExampleCli("getreceivedbyaddress", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\" 0") +
            "\nThe amount with at least 10 confirmation, very safe\n"
            + HelpExampleCli("getreceivedbyaddress", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\" 10") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("getreceivedbyaddress", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\", 10")
       );

    // Bitcoin address
    CYobaCoincoinAddress address = CYobaCoincoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid YobaCoin address");
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount in YOBA received for this account.\n"
            "\nExamples:\n"
...
Value movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 YOBA from the default account to the account named tabby\n"
            + HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 YOBA timotei to akiko with a comment and funds have 10 confirmations\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toyobaaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a YobaCoin address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toyobaaddress\"  (string, required) The YobaCoin address to send funds to.\n"
            "3. amount                (numeric, required) The amount in YOBA. (transaction fee is added on top).\n"
            ...
            "\"transactionid\"        (string) The transaction id.\n"
            "\nExamples:\n"
            "\nSend 0.01 YOBA from the default account to the address, must have at least 1 confirmation\n"
            + HelpExampleCli("sendfrom", "\"\" \"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\" 0.01") +
            "\nSend 0.01 from the tabby account to the given address, funds must have at least 10 confirmations\n"
            + HelpExampleCli("sendfrom", "\"tabby\" \"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\" 0.01 10 \"reward\" \"seans outpost\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("sendfrom", "\"tabby\", \"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\", 0.01, 10, \"reward\", \"seans outpost\"")
        );

    EnsureWalletIsUnlocked();

    string strAccount = AccountFromValue(params[0]);
    CYobaCoincoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid YobaCoin address");
    CAmount nAmount = AmountFromValue(params[2]);
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The YobaCoin address is the key, the numeric amount in YOBA is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            + HelpExampleCli("sendmany", "\"tabby\" \"{\\\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\\\":0.01,\\\"ThiLpx7oYd5YuuhsJAUD5ZsEX2YHgU98Us\\\":0.02}\"") +
            "\nSend two amounts to two different addresses setting the confirmation and comment:\n"
            + HelpExampleCli("sendmany", "\"tabby\" \"{\\\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\\\":0.01,\\\"ThiLpx7oYd5YuuhsJAUD5ZsEX2YHgU98Us\\\":0.02}\" 10 \"testing\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("sendmany", "\"tabby\", \"{\\\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\\\":0.01,\\\"ThiLpx7oYd5YuuhsJAUD5ZsEX2YHgU98Us\\\":0.02}\", 10, \"testing\"")
        );
    ...
    set<CYobaCoincoinAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CYobaCoincoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid YobaCoin address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a YobaCoin address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of YobaCoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) YobaCoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"yobaaddress\"  (string) A YobaCoin address associated with the keys.\n"

            "\nExamples:\n"
            "\nAdd a multisig address from 2 addresses\n"
            + HelpExampleCli("addmultisigaddress", "2 \"[\\\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\\\",\\\"ThiLpx7oYd5YuuhsJAUD5ZsEX2YHgU98Us\\\"]\"") +
            "\nAs json rpc call\n"
            + HelpExampleRpc("addmultisigaddress", "2, \"[\\\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\\\",\\\"ThiLpx7oYd5YuuhsJAUD5ZsEX2YHgU98Us\\\"]\"")
        ;
        throw runtime_error(msg);
    }
    ...
    return CYobaCoincoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CYobaCoincoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CYobaCoincoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CYobaCoincoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in YOBA received by the address\n"
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"yobaaddress\",    (string) The YobaCoin address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in YOBA. This is negative for the 'send' category, and for the\n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
       throw runtime_error(
            ...
            "    \"address\":\"yobaaddress\",    (string) The YobaCoin address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in YOBA. This is negative for the 'send' category, and for the 'move' category for moves \n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"yobaaddress\",   (string) The YobaCoin address involved in the transaction\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending YOBA\n"
            "\nArguments:\n"
...
Value walletlock(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 0))
        throw runtime_error(
            ...
            + HelpExampleCli("sendtoaddress", "\"YYrNgzS7egD4npo8voHNtWPV3VhRpLiYVF\" 1.0") +
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending YobaCoin\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"yobaaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; YobaCoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in YOBA/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new YobaCoin stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid YobaCoin stealth address."));
        return result;
    };
