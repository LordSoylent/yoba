...
int nAnonymizeYobaCoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\YobaCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\YobaCoin
    // Mac: ~/Library/Application Support/YobaCoin
    // Unix: ~/.yobacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "YobaCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "yobacoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "yobacoind.pid"));
