...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeYobaCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeYobaCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeYobaCoinAmount = strAnonymizeYobaCoinAmount.remove(strAnonymizeYobaCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeYobaCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeYobaCoinAmount*COIN) nMaxToAnonymize = nAnonymizeYobaCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeYobaCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeYobaCoinAmount));
        strAnonymizeYobaCoinAmount = strAnonymizeYobaCoinAmount.remove(strAnonymizeYobaCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeYobaCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
